...
def plot_confusion_matrices(ta_labels, ta_preds, gas_labels, gas_preds, save_path=None):
    cm_ta = confusion_matrix(ta_labels, ta_preds, labels=[0, 1, 2, 3])
    cm_gas = confusion_matrix(gas_labels, gas_preds, labels=[0, 1, 2, 3])

    fig, axes = plt.subplots(1, 2, figsize=(10, 4))

    disp_ta = ConfusionMatrixDisplay(cm_ta, display_labels=[0, 1, 2, 3])
    disp_ta.plot(ax=axes[0], values_format="d", colorbar=False)
    axes[0].set_title("TA Confusion Matrix")

    disp_gas = ConfusionMatrixDisplay(cm_gas, display_labels=[0, 1, 2, 3])
    disp_gas.plot(ax=axes[1], values_format="d", colorbar=False)
    axes[1].set_title("GAS Confusion Matrix")
